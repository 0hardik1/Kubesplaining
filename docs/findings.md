...
Each path finding ships with an `escalation_path` array: one `EscalationHop` per step, with `from_subject`, `to_subject`, `action`, `permission`, and a human-readable `gains` line. When exactly one RBAC rule granted the hop, it also carries `source_binding`, `source_role`, and `binding_namespace` naming the (Cluster)RoleBinding and (Cluster)Role behind it (`binding_namespace` is empty for a ClusterRoleBinding). These three are absent on synthetic hops (pod escape, token mint, the node-escape continuation, cloud-identity edges) and on two-rule correlations (secret-mint, node-migrate, csr-approve), where no single binding is responsible for the hop.
