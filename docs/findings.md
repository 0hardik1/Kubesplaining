...
The tool currently emits **73 distinct rule IDs across 11 modules**. Rule IDs are a public surface: they are stable across releases and referenced from `findings.json`, the SARIF output, and the e2e assertions in `scripts/kind-e2e.sh`.
...
| KUBE-PRIVESC-008 | CRITICAL | Impersonation permissions | `impersonate` on users/groups/serviceaccounts. The flat finding fires on the bare grant; the **graph** distinguishes the three, because they are not worth the same thing. `groups` reaches cluster-admin unconditionally (`system:masters` is impersonable whether or not any binding names it, and the apiserver hard-codes it). `users` gets one edge per User subject some binding actually names, since no username is privileged by construction: a stock kubeadm/kind cluster binds its admin identity to the *group* `kubeadm:cluster-admins` and binds no non-`system:` User at all, so there the grant escalates nothing | Grant impersonate only to tightly controlled admin workflows |
| KUBE-PRIVESC-009 | CRITICAL | RBAC `bind` or `escalate` permission | `bind`/`escalate` on roles/clusterroles. The flat finding fires on the bare grant. The **graph** edge additionally requires `create`/`update`/`patch` on roles/clusterroles held by the same subject: `escalate` lifts the "you may only grant what you already hold" rule for role *content*, so without write access to a role there is nothing to widen, and `bind` with no binding to write grants nothing either | Remove from non-admin identities |
| KUBE-PRIVESC-010 | CRITICAL | Role binding modification can self-grant access | create/update/patch on rolebindings/clusterrolebindings. The flat finding fires on the bare grant. The **graph** edge additionally requires `bind` on roles/clusterroles held by the same subject, because the apiserver runs an escalation-prevention check on every binding create *and* update and refuses any binding granting more than the writer holds; `bind` is the documented carve-out. A binding write alone is lateral spread and persistence (hand your own privileges to others), not escalation, and asserting a path from it would be a false positive | Limit binding writes to a tight admin boundary |
| KUBE-PRIVESC-011 | HIGH → CRITICAL | CSR create + approve mints a client cert for any identity | Cluster-scoped `create certificatesigningrequests` AND `update/patch certificatesigningrequests/approval` held by the same subject. CRITICAL when the subject *also* holds `approve` on the `signers` resource for an apiserver-trusted signer, which the CertificateApproval admission plugin has required since 1.19; HIGH without it, since the approval would currently be rejected and the grant is latent. The identity minted is whatever the Subject DN claims: `O=system:masters` is blocked for the kube-apiserver-client signer by CertificateSubjectRestriction, but the Common Name is unrestricted, so `CN=system:serviceaccount:kube-system:<sa>` authenticates as that ServiceAccount | Split the halves across different subjects; reserve approval for the kube-controller-manager auto-approver or a strict admin allowlist, and scope `approve` on `signers` with `resourceNames` |
| KUBE-PRIVESC-024 | HIGH → CRITICAL | Signer control: can issue client certs with no approval step | Cluster-scoped `sign` on the `signers` resource covering an apiserver-trusted client signer AND `update/patch` on `certificatesigningrequests/status` — the pair the CertificateSigning admission plugin enforces for *being* the signer. CRITICAL when the covered signer is `kubernetes.io/kube-apiserver-client` (arbitrary Subject DN); HIGH for the kubelet signers (node identities only). Signer-name matching honors the `resourceNames` grammar: exact name, `kubernetes.io/*`, `*/*`, or unrestricted; a grant pinned to a third-party signer does not fire | Reserve `sign` for the control plane's signer controller; scope `resourceNames` to the one signerName a third-party signer handles, and keep its CA out of the apiserver's `--client-ca-file` |
...
### Certificates ([internal/analyzer/certificates/analyzer.go](../internal/analyzer/certificates/analyzer.go))

The evidence side of the certificates API: these rules read CertificateSigningRequest **objects**, where `KUBE-PRIVESC-011` / `-024` read the RBAC grants. An issued Kubernetes client certificate cannot be revoked (no CRL, no OCSP), so evidence that one was requested outranks the permission to request one.

Two limits are stated in every finding these rules emit. The collector drops the raw `spec.request` PEM by design, so the claimed Subject DN is unknown here — a finding says who asked, never which identity they claimed. And the kube-controller-manager's CSR cleaner deletes issued CSRs about an hour after issuance, so an empty CSR list is not evidence that nothing was minted.

| Rule ID | Sev | Title | What it detects | Remediation |
| --- | --- | --- | --- | --- |
| KUBE-CSR-001 | HIGH (approved) / MEDIUM (pending) | ServiceAccount requested a client certificate | A CSR whose `spec.username` is a ServiceAccount (`system:serviceaccount:<ns>:<name>`) against a signer whose certs authenticate to the apiserver (`kube-apiserver-client`, `kube-apiserver-client-kubelet`, `legacy-unknown`). Denied/Failed requests are skipped. Restricting to ServiceAccount requesters is what keeps the rule quiet: node bootstrap (`system:bootstrap:*` / `system:node:*`) and human cert onboarding fall outside it by construction | Confirm the integration is sanctioned; deny the request; remove `create certificatesigningrequests` from the workload; rotate the cluster CA if a cert was already issued |
| KUBE-CSR-002 | HIGH (approved) / MEDIUM (pending) | CSR targets the deprecated `legacy-unknown` signer | Any CSR with `signerName: kubernetes.io/legacy-unknown`, regardless of requester. CertificateSubjectRestriction only inspects `kube-apiserver-client` CSRs, so this signer is the one route by which an `O=system:masters` request can still be submitted | Deny and delete the request; confirm no controller signs that signerName and no `sign` grant carries a `kubernetes.io/*` or `*/*` wildcard; rotate the CA if a cert was issued |
...
| KUBE-PRIVESC-PATH-NAMESPACE-ADMIN | HIGH (7.6) | `<subject>` can reach namespace-admin in `<ns>` in N hop(s) | Namespace-scoped `create rolebindings` **plus cluster-scoped `bind` on clusterroles** (binding a ClusterRole into a namespace needs `bind` on it, and clusterroles being cluster-scoped, that half can only be granted cluster-wide), or namespace-scoped role write **plus `escalate`**. One sink per affected namespace |
...
| KUBE-CONFUSED-DEPUTY-001 | HIGH | `<subject>` can reach `<sink>` in N hop(s) via an operator it can steer | Any path whose **first** hop is the `operator_reconcile` action. This is a technique overlay, not a distinct sink: the rule ID is substituted in `findingFromPath` when the chain opens with a confused-deputy bridge, and the chain still terminates at whatever the controller itself reaches. One subject reaching several sinks through the same controller produces one finding per sink: each is keyed on its own finding ID, so they no longer collapse into a single finding that discards the rest. These findings carry a structured `RemediationHint` the same way `KUBE-PRIVESC-PATH-*` findings do, since a confused-deputy chain is the same `EscalationPath` shape with a technique overlay on hop 1. Detection: the subject holds `create`/`update`/`patch` on a catalogued operator custom resource AND that operator's controller ServiceAccount exists in the cluster. Catalog and precision gate live in [`internal/analyzer/privesc/deputy.go`](../internal/analyzer/privesc/deputy.go). Remediation: scope the tenant's CR write access, or run the reconciler with impersonation so it acts as the requester rather than as itself. |

Edge techniques that can appear in a hop chain: `KUBE-PRIVESC-001` (pod create), `-002` (pod create into a privileged-allowing namespace → node escape), `-004` (pod exec/attach), `-005`/`-006` (secrets list/read), `-007` (secret create + get → token mint), `-008` (impersonate: the `impersonate` action for groups, `impersonate_system_masters` for the system:masters route, `impersonate_user` for a per-User edge), `-009` (role write + escalate), `-010` (binding write + bind, and the `colocated_sa_token_theft` continuation out of a namespace-admin sink), `-011` (CSR create + approve), `-024` (signer control: `sign` on `signers` + the CSR status write, rated `hard` because the signing CA key is beyond the grant), `-012` (nodes/proxy), `-013` (ephemeral container injection), `-014` (serviceaccounts/token), `-016` (delete pods + node manipulation → node escape), `-017` (wildcard), `KUBE-CONFUSED-DEPUTY-001` (operator reconcile bridge), `KUBE-ESCAPE-CONTROLPLANE-001` (control-plane PKI theft / static-pod admission bypass, emitted only when the snapshot has a control-plane node with no `NoSchedule` taint), plus `KUBE-ESCAPE-00{1,2,3,4,5,6,8}` / `KUBE-HOSTPATH-001` for the pod-escape terminal edge.

Two RBAC-write edges are **conjunctions of two verbs**, held together by one subject but not necessarily granted by one binding, which is why they are built in a per-subject pass (`addRBACWriteEdges`) rather than per rule. Kubernetes runs an escalation-prevention check on every (Cluster)RoleBinding create and update: the writer must already hold every permission the referenced role grants, or hold `bind` on that role, or the API server refuses the write. `bind` and `escalate` are the two documented carve-outs, and each is inert alone. A subject holding only one half still gets its flat `KUBE-PRIVESC-009` / `KUBE-PRIVESC-010` finding; what it does not get is a claimed *path* to a sink the API server would not let it reach.

`system:*` subjects are skipped as traversable intermediates so paths do not launder through the control plane. Non-`system:` ServiceAccounts in `kube-system` / `kube-public` / `kube-node-lease` ARE traversable as intermediates (a co-located controller SA is what real escalations launder through) but are never seeded as path-search sources, which would report the control plane escalating to itself.

Each path finding ships with an `escalation_path` array: one `EscalationHop` per step, with `from_subject`, `to_subject`, `action`, `permission`, and a human-readable `gains` line. When exactly one RBAC rule granted the hop, it also carries `source_binding`, `source_role`, and `binding_namespace` naming the (Cluster)RoleBinding and (Cluster)Role behind it (`binding_namespace` is empty for a ClusterRoleBinding). These three are absent on synthetic hops (pod escape, token mint, the node-escape continuation, cloud-identity edges) and on two-rule correlations (secret-mint, node-migrate, csr-approve, csr-sign), where no single binding is responsible for the hop.

A path finding may also carry an `alternate_escalation_path` array and the tag `privesc:survives-first-cut`, populated by a second BFS pass that re-runs the search per source with the binding behind the first hop banned. A non-empty value is a route to the same sink that survives the fix `RemediationHint` recommends: applying that fix alone does not close the path. An empty value (the common case) means one of three things: no such route exists within the searched depth; the first hop is synthetic so no binding cut was modeled; or a surviving route exists but is longer than `--max-privesc-depth` (default 5), since the alternate search shares the primary search's depth bound while doing a strictly harder job.
