...
func hopNarrative(hop models.EscalationHop) string {
	...
	case "csr_approve":
		return fmt.Sprintf("Acting as %s, the attacker submits a CertificateSigningRequest whose Subject DN claims a privileged identity, then self-approves it via the `certificatesigningrequests/approval` subresource (%s). The apiserver maps a client cert's Common Name onto the username it authorizes and never checks whether that identity is one Kubernetes would have issued, so `CN=system:serviceaccount:kube-system:<sa>` authenticates as that ServiceAccount. (The textbook `O=system:masters` claim is blocked for this signer by the default CertificateSubjectRestriction admission plugin; the CN route is not.) The cert persists after the RBAC grant is revoked; with no CRL in Kubernetes, only a CA rotation invalidates it.", from, perm)

	case "csr_sign":
		return fmt.Sprintf("Acting as %s, the attacker skips approval entirely: it holds `sign` on the `kubernetes.io/kube-apiserver-client` signer plus the `certificatesigningrequests/status` write (%s), which is the authorization Kubernetes defines for *being* the signer. It issues a client certificate for any identity it names and writes the result onto the CSR status itself. The one thing beyond the grant is the signing CA key — the identity this grant designates holds it by definition, which is why the hop is rated hard rather than assumed.", from, perm)
